...
def _format_bounds(bounds_list, *arg, width=None, **kw):
    ...
    for name, kind, dom in bounds_list:
        ...
        if isinstance(dom, Nodes.Domain):
            ...
            bound = "{name} \\in {b}".format(name=name, b=bound)
            old_dom = dom
...
class Nodes(_Nodes):
    ...
    class Apply(_Nodes.Apply):
        def to_str(self, *arg, width=None, **kw):
            ...
            if isinstance(fixity, _optable.Infix):
                ...
                res = ("{arg1} {op} {arg2}").format(
                    op=op_str, arg1=arg_strings[0], arg2=arg_strings[1]
                )
                res_width, _ = _box_dimensions(res)
                if res_width > width:
                ...
                return ("{op} {arg}").format(op=op_str, arg=arg_strings[0])
            elif isinstance(fixity, _optable.Postfix):
                assert len(arg_strings) == 1, arg_strings
                return ("{arg}{op}").format(op=op_str, arg=arg_strings[0])
            else:
                pass
            args_str = ", ".join(arg_strings)
            res = "{op}({args})".format(op=op_str, args=args_str)
            res_width, _ = _box_dimensions(res)
            if res_width > width:
                args_str = ",\n".join("    " + arg for arg in arg_strings)
                res = "{op}(\n{args})".format(op=op_str, args=args_str)
            return res
    ...
    class TemporalSub(_Nodes.TemporalSub):
        def to_str(self, *arg, **kw):
            ...
            if isinstance(self.op, Nodes.BoxOp):
                res = "[][{a}]_{sub}".format(a=action, sub=sub)
    ...
    class Dot(_Nodes.Dot):
        def to_str(self, *arg, width=None, **kw):
            ...
            return "{expr}.{string}".format(expr=expr, string=string)
    ...
    class If(_Nodes.If):
        def to_str(self, *arg, width=None, **kw):
            ...
            res = ("IF {test} THEN {then} " "ELSE {else_}").format(
                test=test, then=then, else_=else_
            )
            res_width, _ = _box_dimensions(res)
            if res_width > width:
    ...
    class Record(_Nodes.Record):
        def to_str(self, *arg, width=None, **kw):
            ...
            for name, e in self.items:
                expr = e.to_str(*arg, width=width, **kw)
                item = "{name} |-> {expr}".format(name=name, expr=expr)
                items.append(item)
            items_str = ", ".join(items)
            res = "[{items}]".format(items=items_str)
            return res
    ...
    class Number(_Nodes.Number):
        def to_str(self, *arg, **kw):
            if self.mantissa is None:
                ...
                return "{i}.{m}".format(i=self.integer, m=self.mantissa)
    ...
    class Flex(_Nodes.Flex):
        def to_str(self, *arg, width=None, **kw):
            return "VARIABLE {name}".format(name=self.name)
    ...
    class Theorem(_Nodes.Theorem):
        def to_str(self, *arg, width=None, **kw):
            ...
            if self.name is None:
                ...
                theorem = "THEOREM {name} ==".format(name=self.name)
