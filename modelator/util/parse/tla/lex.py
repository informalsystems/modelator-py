...
class Lexer:
    ...
    #   { [ PUNCT p ] }
    punctuation = (
        # The symbols dot '.', colon `:`, and at `@`
        # have been moved below to the methods
        # `PUNCTUATION_DOT` (`.`),
        # `PUNCTUATION_COLON` (`:`),
        # `PUNCTUATION_AT` (`@`)
        r",|\(|\)|\[|\]_|\]|\{|\}|<<|>>_|>>|==|!"
    ...
    # ensure that ':>' appears before `:`
    def t_PUNCTUATION_COLON(self, t):
        r":"
        t.type = "PUNCTUATION"
        return t

    # ensure that `@@` appears before `@`
    def t_PUNCTUATION_AT(self, t):
        r"\@"
        t.type = "PUNCTUATION"
        return t
...
def find_column(input, token):
    ...
    if column < 1 or column > n_chars:
        raise AssertionError(
            f"expected `column \in 1..{n_chars} " f"(computed `{column = }`)"
        )
    return column
